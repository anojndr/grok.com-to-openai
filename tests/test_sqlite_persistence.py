...
class SqlitePersistenceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_restart_recovery_for_chat_completions(self):
        ...
        with patch("server.run_session_turn", new=fake_run_turn), \
             patch("server.refresh_statsig_pair", new=AsyncMock()):
            resp = await server.chat_completions(FakeRequest({
    ...
    async def test_branch_uses_parent_checkpoint_not_later_sibling(self):
        ...
        with patch.object(server, "run_session_turn", new=fake_run), \
            ...
            for users in ([u1], [u1, u2], [u1, u2, u3]):
                resp = await server.chat_completions(
                    FakeRequest({"messages": [{"role": "user", "content": user} for user in users]}))
                self.assertEqual(resp.status_code, 200)

            # Turn 4: in-memory branch from Turn 2 [u1, u2]
            resp = await server.chat_completions(
                FakeRequest({"messages": [{"role": "user", "content": user} for user in [u1, u2, u4]]}))
            self.assertEqual(resp.status_code, 200)

            # Turn 5: cold-start branch from Turn 2 [u1, u2] after clearing in-memory SESSIONS
            server.SESSIONS.clear()
            resp = await server.chat_completions(
                FakeRequest({"messages": [{"role": "user", "content": user} for user in [u1, u2, u5]]}))
            self.assertEqual(resp.status_code, 200)

        # Assert correct parent response IDs seen by fake_run for each prompt
        self.assertEqual(seen_parents, [
            (u1, ""),
            (u2, "response-1"),
            (u3, "response-2"),
            (u4, "response-2"),  # in-memory branch correctly attached to Turn 2 output
            (u5, "response-2"),  # cold-start SQLite branch correctly attached to Turn 2 output
        ])
        ...
        self.assertEqual(self.store.get_session(parent_key)["last_parent_response_id"], "response-2")
        self.assertEqual(self.store.get_session(sibling1_key)["last_parent_response_id"], "response-3")
        self.assertEqual(self.store.get_session(sibling2_key)["last_parent_response_id"], "response-4")
        self.assertEqual(self.store.get_session(sibling3_key)["last_parent_response_id"], "response-5")
